-- SQL skripta za postavljanje RLS politika za tabele grejanje i investitor
...
-- Kreiraj RLS politike za tabele
SELECT create_rls_policies_for_table('grejanje');
SELECT create_rls_policies_for_table('investitor');
...
SELECT 
  ...
  cmd as command
FROM pg_policies 
WHERE tablename IN ('grejanje', 'investitor')
